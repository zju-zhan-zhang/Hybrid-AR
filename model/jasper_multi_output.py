...
class JasperEncoder(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        activation = jasper_activations[cfg['activation']]()
        self.use_conv_mask = cfg.get('convmask', False)
...
class Jasper(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.jasper_encoder = JasperEncoder(**kwargs.get("encoder"))
        self.jasper_decoder = JasperDecoderForCTC(**kwargs.get("decoder"))
    ...
    def forward(self, x):
        ...
        a_inp = x
        r = self.acoustic_model.forward(a_inp)
        # in accord with wav2letter output shape
        r["final_output"] = r["final_output"].transpose(1, 2)
        return r
...
class JasperAcousticModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.jasper_encoder.use_conv_mask:
            ...
            return {"encoder_output":t_encoded_t[0], "final_output":out}
...
if __name__ == "__main__":
    cfg = yaml.load(open("jasper-5x3.yaml", "r"))
    model = Jasper(**cfg).cuda()
    # (bs, mfcc, timesteps)
    # summary(model, (39, 500))
    s = torch.randn(4, 39, 500).cuda()
    out = model(s)
    print(out.shape)
